...
sidebar:
    order: 13
slug: 0.10.0/configuration/tabs
---
...
Check out the [default config](/0.10.0/reference/config/) for reference.
...
- `Pane(<pane>)` - Displays the given pane type itself. Available panes are listed in the [panes](/0.10.0/configuration/panes) page.
- `Split(direction: Horizontal | Vertical, borders: <borders>, panes: <sub_pane[]>)` splits the pane into multiple [sub pane](#sub_pane)s. Each split has
  a direction and a list of sub panes. More info on borders in [their section](#borders).
- `Component("<component_name>")` - Displays a [component](/0.10.0/configuration/layout#components)
...
<ConfigValue name="panes" customText="(size: '50%', borders: <borders>, pane: Pane | Split | Component" />
...
<ConfigValue
    ...
    href="/0.10.0/configuration/header/#header_property_kind"
/>
