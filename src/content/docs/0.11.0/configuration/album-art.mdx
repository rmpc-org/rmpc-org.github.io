...
sidebar:
  order: 20
slug: 0.11.0/configuration/album-art
---
...
Some of the terminals do not correctly report their size, thus some tweaking is needed. For those you need to set [max\_size\_px](#max_size_px).
...
| Ghostty   | Kitty                | yes  | -                                                                                                                                                                    |
| WezTerm   | ITerm2               | yes  | -                                                                                                                                                                    |
| vscode    | ITerm2               | yes  | Needs to set max\_size\_px. Needs to be enabled in vscode.                                                                                                             |
| Tabby     | ITerm2               | yes  | Needs to set max\_size\_px                                                                                                                                             |
...
| Others    | Ueberzug/Sixel/Kitty | yes  | Untested/unsupported. Might need [ueberzugpp](https://github.com/jstkdng/ueberzugpp). Try other backends or disabling album\_art altogether if you experience issues. |

## Configuration

### method

<ConfigValue name="method" type={["Kitty", "Iterm2", "Sixel", "UeberzugWayland", "UeberzugX11", "Block", "None", "Auto"]} />
...
For all terminals, `Block` is now supported. It will fall\_back to ANSI if the terminal doesn't support truecolor.

`None` completely disables album art display.

### order

<ConfigValue name="order" type={["EmbeddedFirst", "FileFirst", "EmbeddedOnly", "FileOnly"]} />
...
### default\_album\_art\_path

<ConfigValue name="default_album_art_path" type="string" optional />

The default album art image can also be changed [in your theme](/0.11.0/configuration/theme#default_album_art_path).
...
* Much lower image size can be transmitted through tmux due to sixel being a very size inefficient protocol.
  ...
  You can limit your image size with [max\_size\_px](#max_size_px).
