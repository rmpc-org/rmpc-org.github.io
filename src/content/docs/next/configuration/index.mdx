...
Template string for formatting durations throughout the application. Uses `%` tokens for time components.
...
:::note
Some properties like `QueueTimeTotal` and `QueueTimeRemaining` can override the format when a separator is specified in the property definition.
:::
