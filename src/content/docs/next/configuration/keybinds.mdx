...
<ConfigValue
    name="keybinds"
    type="other"
    customText="(clear: <bool>, normal_timeout_ms: <number>, insert_timeout_ms: <number>, global: <kebyinds_map>, navigation: <keybinds_map>, queue: <keybinds_map>)"
/>
...
## normal_timeout_ms and insert_timeout_ms

<ConfigValue name="normal_timeout_ms" type="number" />
<ConfigValue name="insert_timeout_ms" type="number" />

How long will rmpc wait before resolving the currently queued inputs. The timeout can be configured
separately for insert mode (while typing inside an input) and normal mode (everywhere else). Insert mode
only respects the `Close` and `Confirm` keybinds.
