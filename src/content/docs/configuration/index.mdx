...
<ConfigValue name="address" type={['"<ip>:<port>"', '"/path/to/socket"', '"@abstract_socket"']} />

The address to connect to. Address can be either an IP address with a port or a UNIX domain socket path. On Linux specifically, it's also possible to use abstract socket with a prefix of `@`.
...
2. `$MPD_HOST` and `$MPD_PORT` environment variables
    ...
    - Password can be provided via `$MPD_HOST` in the `password@host` format. For abstract socket, it should look like `password@@abstract_socket`.
...
Directory where rmpc should search for `lrc` files. Please see the [lyrics page](/configuration/lyrics) for more information.
...
<ConfigValue name="enable_lyrics_index" type="bool" />

Whether rmpc should index your `lyrics_dir` to be able to match lyrics to `.lrc` by the metadata
(artist, title, album, length). Disabling this also disables `enable_lyrics_hot_reload`. Only disable
this if you experience performance issues with very large number of files in your `lyrics_dir`.
Defaults to `true`.

### enable_lyrics_hot_reload

<ConfigValue name="enable_lyrics_hot_reload" type="bool" />

Whether rmpc should watch the [lyrics_dir](/configuration/#lyrics_dir) for changes and automatically load new files or update lyrics from changed files. Disabled by default.
...
<ConfigValue name="enable_keepalive" type="bool" />

Whether to enable TCP keepalive on the MPD socket. When enabled, the OS will periodically send keepalive probes on an
idle connection to detect and clean up stale connections. Only applies when connecting via TCP (not Unix sockets).
Enabled by default.
...
### keep_state_on_song_change

<ConfigValue name="keep_state_on_song_change" type="bool" />
...
<ConfigValue name="auto_open_downloads" type="bool" />

Whether to automatically open the downloads modal when starting a new ytdlp download.
Defaults to `true`.

### quit_closes_modal

<ConfigValue name="quit_closes_modal" type="bool" />

If true, `Quit` action will close an opened modal. Defaults to `false`.

### extra_yt_dlp_args

<ConfigValue name="extra_yt_dlp_args" type="string[]" />

Additional arguments that will be passed to yt-dlp when downloading. Useful to pass in for example
`["--cookies-from-browser=firefox"]`.

Rmpc executes the following:

```bash frame=none showLineNumbers=false
yt-dlp -x --embed-thumbnail --embed-metadata -f bestaudio --convert-thumbnails jpg {} --output <path> <url>
```

The `extra_yt_dlp_args`, if any, will be passed instead of the {} in the command.

:::caution
Only use this if you are really sure you know what you are doing!
:::

### queue_disable_current_item_style_timeout_ms

<ConfigValue name="queue_disable_current_item_style_timeout_ms" type="number" optional />

Disables the highlight with [current_item_style](/configuration/theme#current_item_style) after the
timeout elapses. The highlight will re-enable with the next keypress and the timeout will restart.

### duration_format

<ConfigValue name="duration_format" type="string" optional />
...
:::caution
Invalid format tokens (e.g., `%z`) or a trailing `%` will cause a config error. Only the tokens listed above are valid.
:::
...
<ConfigValue name="album_art" type="other" href="/configuration/album-art" />

Various options for album art display. By default should not be needed to be specified. More info at [album art page](/configuration/album-art#configuration)

### cava

<ConfigValue name="cava" type="other" customText="<Cava>" href="/configuration/cava" />
...
More info on the [cava](/configuration/cava) page.

### keybinds

<ConfigValue name="keybinds" type="other" customText="<keybinds>" href="/configuration/keybinds/" />

Keybinds to use. The default keybinds are used if this property is absent. More info at:

<LinkCard title="Configuring keybinds" description="Customize the keybinds" href="/configuration/keybinds/" />

### normal_timeout_ms

<ConfigValue name="normal_timeout_ms" type="number" />

How long will rmpc wait before resolving the currently queued inputs. This timeout is for normal
mode (basically everywhere except text inputs).

This timeout only comes into play when a key sequence is ambiguous. When the sequence directly resolves
to an action it is executed immedietally.

Defaults to to `1000`.

#### Example

Consider following keymap:
...
If you press `g` nothing will happen but after the timeout elapses the action will resolve as `Bottom`.
If you press `g` and `g` in quick succession the action will resolve as `Top`. If you press `g` and
some other key for example `z` in quick succession nothing will happen and the input buffer will be
cleared.

### insert_timeout_ms

<ConfigValue name="insert_timeout_ms" type="number" />

How long will rmpc wait before resolving the currently queued inputs. This timeout is for insert mode
(while typing inside an input). Insert mode only respects the `Close` and `Confirm` keybinds.
...
<ConfigValue name="scroll_amount" type="number" />

Amount of lines each scroll wheel move scrolls by. Defaults to 1.

### enable_focus_events

<ConfigValue name="enable_focus_events" type="bool" />

Rmpc will handle focus events when enabled. Defaults to `true`. Only disable this if you are
experiencing issues.
...
send [notifications](/guides/on_song_change) when the song changes.
...
arguments passed to the command. Nothing will be executed if left empty. Check [on_resize](/guides/on_resize)
for more info.

### on_exit

<ConfigValue name="on_exit" type="other" customText={'["rmpc", "--address", "/path/to/socket", "stop"]'} />

Execute given command when rmpc exits. First element of the array is the command to execute and the rest are the
arguments passed to the command. Nothing will be executed if left empty.
...
<ConfigValue name="search" type="other" customText="<search>" href="/configuration/search/" />

Configures what tags are searchable on the Search pane. Also sets whether the search should be case insensitive by
default and what kind of search mode to perform. More detail in the [Search](/configuration/search) section.
If not specified, the [default values](/reference/config) are used.
...
Albums can display either only its name or include an album's date as well. Additionally, albums
can be sorted by their name or date.
...
<ConfigValue name="ignore_leading_the" type="boolean" />

Leading "The" will be ignored while sorting in browser panes if set to true. Defaults to `false`
if not specified.
...
Defines how the songs are sorted in the browser panes. This is a list of [song properties](/configuration/properties/#song-properties).
...
- `SortFormat` - Similar to `Format` but songs are sorted by [browser_song_sort](/configuration/#browser_song_sort).
...
<ConfigValue name="directories_hidden_dirs" type="other" customText="string[]" />

A list of directory names to hide from the Directories pane. Directories are matched by their
name at any depth, for example `[".Trash-1000"]` hides every directory named `.Trash-1000`.
Defaults to `[]`.
