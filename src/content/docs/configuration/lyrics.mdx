...
Rmpc supports displaying synchronized lyrics in the `Lyrics` pane. Supported formats are currently
[LRC](<https://en.wikipedia.org/wiki/LRC_(file_format)>) and the new
[lyricsfile](https://lrclib.net/lyricsfile) format. Other lyrics formats are not supported. The
`lyrics_dir` must be configured for the resolution to work. All `lrc` files must be on the client
side.

If you do not know which format to choose, choose `lyricsfile`. Its spec is in its infancy but is
already a huge improvement over the `lrc` which does not even have a formal specification and is
...
Lrc files are resolved by rmpc via three methods (in order):

- Same path as the song file, except with the `.lyricsfile.yaml` file extension, ie. `track.mp3`
  would become `track.lyricsfile.yaml`
- Same path as the song file, except with the `.lrc` file extension, ie. `track.mp3` would become
  `track.lrc`
- By indexing all the `.lrc` files in the `lyrics_dir`

#### Lyrics index

Rmpc will create an index of all `.lrc` (`lyricsfile`s are currently not indexed) files in your
`lyrics_dir` on startup.
